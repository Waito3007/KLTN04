...
def train_epoch(model, dataloader, optimizers, criteria, device):
    """Train for one epoch"""
    model.train()
...
def evaluate_model(model, dataloader, criteria, device):
    ...
    for task in criteria.keys():
        if predictions[task]:
            ...
            metrics[task] = {
                'loss': total_losses[task] / len(dataloader),
                'accuracy': accuracy
            }
...
def main():
    ...
    for epoch in range(num_epochs):
        ...
        # Train
        train_losses, train_total_loss = train_epoch(model, train_loader, optimizers, criteria, device)
        ...
        avg_val_accuracy = val_accuracy_sum / len(val_metrics)
